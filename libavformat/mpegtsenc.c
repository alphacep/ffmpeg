...
static uint8_t rs_d2i[256];  //map symbols to index form
static uint8_t rs_g[17] = {1, 118, 52, 103, 31, 104, 126, 187, 232, 17, 56, 183, 49, 100, 81, 44, 79};
static uint8_t rs_q;
static uint16_t rs_ind, rs_ind1, rs_ind2;
...
void rs255239_init(void) {
    ...
    //note that rs_i2d[i] belongs to 1..255
    //rs_d2i[0] is a padding slot for operation reduction
    for(int i = 0; i < 255; i++) {
        rs_d2i[rs_i2d[i]] = (uint8_t)i;
    }
}
...
void rs255239_pdiv(void) {
    ...
    for(int i = 0; i < 239; i++) {
        if(rs_x[i] == 0x0) {
            ...
            rs_ind1 = (uint16_t) rs_d2i[rs_x[i]];
            for(int j = 0; j < 17; j++) {
                //convert from decimal to index
                rs_ind2 = (uint16_t) rs_d2i[rs_g[j]];
                rs_ind  = rs_ind1 + rs_ind2;
                rs_ind  = rs_ind % 255;
                rs_q  = rs_i2d[rs_ind];
                rs_x[i+j] ^= rs_q;
            }
...
static void write_packet(AVFormatContext *s, const uint8_t *packet)
{
    ...
    // on RS 255/239 encoder
    char encpacket[256];
    memset(rs_x, 0, 256);
    memset(encpacket, 0, 256);
    memcpy(rs_x + 51, packet, TS_PACKET_SIZE);
    memcpy(encpacket + 51, packet, TS_PACKET_SIZE);
    rs255239_pdiv();
    memcpy(encpacket + 239, rs_x + 239, 16);
...
const uint8_t goinv[128] = {

0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd,
0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19,
0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f,

0x20, 0x21, 0x22, 0x5f, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b,
0x2c, 0x2d, 0x2e, 0x2f, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37,
0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f, 0x40, 0x41, 0x42, 0x43,
0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f,
0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x20,
0x20, 0x20, 0x5e, 0x20, 0x20, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67,
0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73,
0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x20, 0x20, 0x20, 0x20, 0x7f

};

uint8_t teletextToUcs2(uint8_t chr);
